...
if [ $CURL_RC -eq 0 ] && [ $LOGIN_RC -eq 0 ] && [ $WHOAMI_RC -eq 0 ]; then
    printf '\033[1;32m✅ Done:\033[0m Secrets Manager CLI installed and logged in.\n'
    printf '\033[1;33m➡️  Next:\033[0m run ./07.loading-demo-data.sh\n'
else
    printf '\033[1;31m❌ Failed:\033[0m Secrets Manager CLI install/login failed - check the output above.\n'
fi
