...
# The full "watch it rotate" story only makes sense once all 8 demo
# variants are actually deployed - warn and confirm if some are missing,
# rather than silently rotating against a partial lab.
DEMO_DEPLOYMENTS="cityapp:cityapp-hardcode cityapp:cityapp-conjurtok8sfile cityapp:cityapp-conjurtok8ssecret cityapp:cityapp-conjurtok8ssecret-init cityapp:cityapp-springboot-sidecar cityapp:cityapp-springboot-native external-secrets:cityapp-eso cityapp:cityapp-csi"
FOUND_COUNT=0
MISSING=""
...
if [ $FOUND_COUNT -lt 8 ]; then
    printf '\033[1;33m⚠️  Warning:\033[0m only %s/8 cityapp demo deployments found. Missing:%s\n' "$FOUND_COUNT" "$MISSING"
...
if [ $MYSQL_RC -eq 0 ] && [ $HOST1_RC -eq 0 ] && [ $HOST2_RC -eq 0 ]; then
    if [ "$MODE" = "all" ]; then
        ...
        printf '\033[1;33m➡️  Next:\033[0m cityapp-conjurtok8sfile and cityapp-conjurtok8ssecret refresh live within their secrets-refresh-interval, no redeploy needed. cityapp-conjurtok8ssecret-init, cityapp-springboot-sidecar, cityapp-springboot-native and cityapp-csi need a redeploy to pick it up. cityapp-hardcode and cityapp-eso are now both stuck on the old password: hardcode because it never talks to Secrets Manager at all, eso because it reads test/host2/* which this script left alone by default - run this script again with "host2" to repair just that, or "all" next time to rotate both together from the start.\n'
    fi
else
