...
case "$MODE" in
    ...
    *)
        ...
        echo "  all: rotate the real MySQL password and update both test/host1/pass and test/host2/pass together - every Conjur-integrated variant keeps working."
        exit 1
        ;;
esac
...
# cityapp-conjurtok8ssecret, cityapp-springboot-sidecar,
# cityapp-springboot-native and cityapp-csi all read test/host1/* and keep
# working (live, or after a redeploy - see README.md PART IV for exactly
# which apps need which). cityapp-eso reads the separate test/host2/* copy
# (see 5.conjur-eso/01.adding-conjur-eso-policy.sh) and is intentionally
# left behind here, right alongside cityapp-hardcode which never talks to
# Conjur at all - both go stale, everything else survives the rotation.
...
if [ $MYSQL_RC -eq 0 ] && [ $HOST1_RC -eq 0 ] && [ $HOST2_RC -eq 0 ]; then
    if [ "$MODE" = "all" ]; then
        printf '\033[1;32m✅ Done:\033[0m %s password rotated in MySQL and updated in Conjur (test/host1/pass and test/host2/pass).\n' "$DB_USER"
        printf '\033[1;33m➡️  Next:\033[0m cityapp-conjurtok8sfile and cityapp-conjurtok8ssecret refresh live within their secrets-refresh-interval, no redeploy needed. cityapp-springboot-sidecar (secret wired up as an env var, frozen at pod start), cityapp-springboot-native (fetches once via the Conjur SDK at startup) and cityapp-csi (rotation is not enabled on this lab'"'"'s CSI driver install) all need a redeploy to pick it up. cityapp-hardcode is the only one still stuck on the old password now, since it never talks to Conjur at all.\n'
    else
        ...
        printf '\033[1;33m➡️  Next:\033[0m cityapp-conjurtok8sfile and cityapp-conjurtok8ssecret refresh live within their secrets-refresh-interval, no redeploy needed. cityapp-springboot-sidecar, cityapp-springboot-native and cityapp-csi need a redeploy to pick it up. cityapp-hardcode and cityapp-eso are now both stuck on the old password: hardcode because it never talks to Conjur at all, eso because it reads test/host2/* which this script left alone by default - run this script again with "host2" to repair just that, or "all" next time to rotate both together from the start.\n'
    fi
else
